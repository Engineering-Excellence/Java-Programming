...
//	상수를 초기화 시키는 방법은 3가지가 있는데 1번째 방법은 상수 선언시 "=" 사용해서
//	초기화 시키는 방법이고 2번째 방법은 객체가 생성될 때 생성자에서 초기화 시키는 
//	방법이 있다.
//	3번째 방법은 static으로 선언한 상수를 초기화는데 2문장 이상 필요한 경우
//	static { }와 같이 초기화 블록을 사용하는 방법이 있다.
    public final int MAX_HP; // 최대 HP, 상수
    public int currentHP; // 현재 HP

    //	생성자를 선언하지 않으면 자바 컴파일러가 아무런 일도 하지않는 기본 생성자를 만들어
//	주지만 다른 생성자를 선언하면 기본 생성자를 자동으로 만들어 주지 않는다.
...
//		표시 인터페이스 타입으로 수리할 객체를 받았지만 표시 인터페이스는 아무런 내용이
//		코딩되어있지 않기 때문에 인수로 넘겨받은 객체를 실제로 구현한 내용이 들어있는
//		클래스로 형변환 시킨 후 처리해야 한다.
        if (repairable instanceof Unit unit) {
