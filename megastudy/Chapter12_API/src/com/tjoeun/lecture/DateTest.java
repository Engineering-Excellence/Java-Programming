...
//		Date 클래스는 0 ~ 11월로 월을 처리하므로 얻어와서 1을 더해야 한다.
...
//		Calendar 클래스 객체를 이용해서 날짜/시간을 출력할 서식을 적용하려면 gettime() 메소드를
//		실행해서 날짜/시간 정보만 얻어온 후 서식을 적용해야 한다.
...
//		currentTimeMillis() 메소드의 실행 결과는 13자리으 정수이므로 int 타입의 변수에 저장시킬 수 없고
...
//		실행할 경우 별 문제가 발생되지 않지만 연산 결과를 SimpleDateFormat 클래스 객체를 사용해
//		서식을 지정하면 offset 값이 포함된 시간으로 서식이 적용된다.
//		=> 연산 결과에서 offset 값을 빼면된다.
