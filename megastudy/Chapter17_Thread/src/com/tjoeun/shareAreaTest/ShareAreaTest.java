...
//		기본 자료형으로 선언한 변수는 일반 변수, 클래스로 선언한 변수(객체, 인스턴스) 참조 변수라 부른다.
...
//		공유 영역을 만드는 2번째 방법
//		공유 영역을 공유해서 사용할 클래스의 객체를 선언할 때 생성자의 인수로 공유 영역의 주소를
//		넘겨준다.
//		CalculatorThread calculator = new CalculatorThread(shareArea);
//		PrintThread print = new PrintThread(shareArea);

//		공유 영역을 만드는 3번째 방법
//		공유 영역을 공유해서 사용할 클래스의 객체를 선언하고 setter 메소드를 사용해서 공유 영역의
//		주소를 넣어준다.
        CalculatorThread calculator = new CalculatorThread();
        PrintThread print = new PrintThread();
