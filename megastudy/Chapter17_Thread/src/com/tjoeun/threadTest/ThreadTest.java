...
//		2. Thread 클래스를 상속받아 작성된 클래스 객체에서 start() 메소드로 멀티 스레드를
//		   실행한다.
//		아래와 같이 run() 메소드를 Override 했다고 해서 run() 메소드를 실행하면 run()이라는
//		일반 메소드가 실행된다.
//		digit.run();
//		멀티 스레드로 실행하려면 반드시 start() 메소드를 사용해서 실행해야 한다.
        digit.start();
...
//		2. Runnable 인터페이스는 스레드를 실제로 실행하는 기능이 없으므로 스레드를 실행하기 
//		   위해서 Thread 클래스를 이용해야 한다.
//		   Thread 클래스의 객체를 생성할 때 생성자의 인수로 Runnable 인터페이스를 구현받아
//		   멀티 스레드를 작성한 클래스의 객체를 넘겨준다.
