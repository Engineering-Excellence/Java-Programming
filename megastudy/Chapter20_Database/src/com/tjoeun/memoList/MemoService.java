...
//		후처리
    ...
    //	테이블에 저장된 메모 목록을 글번호(idx)의 내림차순(최신글 순)으로 얻어오는
//	DAO 클래스에 메소드를 호출하는 메소드
    public static void select() {
//		테이블에 저장된 메모 목록을 얻어온다.
        List<MemoVO> list = MemoDAO.select();
...
//		얻어온 메모 목록을 출력한다.
    ...
    //	삭제할 글번호를 입력받고 비밀번호를 확인한 다음에 글을 삭제하는 DAO 클래스에
//	메소드를 호출하는 메소드
...
//		테이블에서 삭제할 글을 얻어오는 DAO 클래스에 메소드를 호출한다.
...
//				비밀번호가 일치하면 메모를 삭제하는 DAO 클래스에 메소드를 호출한다.
                MemoDAO.delete(idx);
    ...
    //	수정할 글번호를 입력받고 비밀번호를 확인한 다음에 글을 수정하는 DAO 클래스에
//	메소드를 호출하는 메소드
...
//		테이블에서 수정할 글을 얻어오는 DAO 클래스에 메소드를 호출한다.
        MemoVO original = MemoDAO.selectByIdx(idx);
//		System.out.println(original);
...
//				비밀번호가 일치하면 메모를 수정하는 DAO 클래스에 메소드를 호출한다.
