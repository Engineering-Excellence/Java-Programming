...
INSERT INTO memo(NAME, PASSWORD, memo)
VALUES ('임꺽정', '2222', '2등 입니다.');
INSERT INTO memo(NAME, PASSWORD, memo)
VALUES ('장길산', '3333', '3등 입니다.');
INSERT INTO memo(NAME, PASSWORD, memo)
VALUES ('일지매', '4444', '4등 입니다.');
INSERT INTO memo(NAME, PASSWORD, memo)
VALUES ('길홍동', '5555', '5등 입니다.');
INSERT INTO memo(NAME, PASSWORD, memo)
VALUES ('홍동길', '6666', '6등 입니다.');

#
SELECT [DISTINCT] 필드이름 또는 *(모든 필드)
FROM 테이블이름;
SELECT *
FROM memo;
#
...
# 정렬 방식 생략시 ASC가 기본값으로 사용되며 오름차순으로 정렬되고 DESC를 
# 사용하면 내림차순으로 정렬된다.
SELECT * FROM memo ORDER BY NAME; #
name 필드를 오름차순으로 정렬한다.
SELECT * FROM memo ORDER BY NAME ASC;
SELECT *
FROM memo
ORDER BY NAME DESC;
#
name 필드를 내림차순으로 정렬한다.
...
WHERE idx >= 15 && idx <= 20;
#
BETWEEN 연산자는 ~이상이고 ~이하인 데이터를 얻올 수 있다. 이상, 이하 조건만 가능하다.
...
# '종로_가'는 종로1가, 종로2가, 종로3가, 종로4가, 종로5가, 종로6가 처럼 '_' 자리에 어떤
# 문자가 나와도 상관없다.
...
# index는
select sql 명령을 실행했을 때 출력 결과의 1번째 데이터의 index가 0부터 시작된다.
...
#
그룹 함수를 실행한 결과나 필드 이름을 다른 이름으로 변경해서 보려면 'as' 예약어를
# 사용해서 별명을 지정할 수 있다.
...
HAVING COUNT(*) >= 10;
#
name과 개수를 memo 테이블에서 name에 '길'을 포함하는 name별로 그룹화해서 그룹화된 데이터의
# 개수가 10개 이상인 것만 검색
SELECT NAME, COUNT(*)
FROM memo
WHERE NAME LIKE '%길%'
