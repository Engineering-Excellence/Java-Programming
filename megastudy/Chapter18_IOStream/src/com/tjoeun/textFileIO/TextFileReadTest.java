...
//		파일의 경로와 이름을 설정한다.
//		텍스트 파일을 읽어오지 못할 경우 텍스트 파일을 열고 다른 이름으로 저장을 실행해서 인코딩을
//		ANSI에서 UTF-8로 변경하면 읽어들일 수 있다.
        String filePath = "./Chapter18_IOStream/resources/in.txt";
//        String filePath = "./Chapter18_IOStream/resources/out.txt";

        try (Scanner scanner = new Scanner(new File(filePath))) {
//			Scanner 클래스 객체를 만들때 생성자로 "System.in"을 넘겨주면 키보드로 입력받는
//			스캐너가 만들어진다.
//			텍스트 파일에서 데이터를 입력받는 스캐너를 만들려면 읽어들이려는 텍스트 파일의 경로와
//			이름으로 File 클래스 객체를 만들어 Scanner 생성자로 넘겨줘야 한다.
...
//			실행해서 파일을 닫아야 하지만 입력 용도로 사용하는 Scanner 객체는 파일을 닫지
//			않아도 정상적으로 처리된다.
    }
}
