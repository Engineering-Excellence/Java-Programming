...
//		super()는 코딩하지 않더라도 자바 컴파일러가 자동을 만들어준다.
//		this()와 super()는 반드시 생성자의 첫 문장으로 써야한다. => 가장 먼저 실행된다.
//		부모 클래스의 생성자가 자식 클래스의 생성자보다 먼저 실행된다.
//		부모 클래스의 기본 생성자를 정의하지 않으면 자식 클래스에서 super()를 실행할 때
//		에러가 발생된다.
//		super();
...
//	부모 클래스에서 상속받은 private 권한의 필드를 초기화하는 방법은 부모 클래스에서 상속받은
//	필드의 접근 권한이 private일 경우 자식 클래스에서 접근할 수 없으므로 부모 클래스의 생성자를
//	호출해서 초기화시킨다.
...
//	부모 클래스에 private 권한으로 설정된 모든 필드의 setter가 정의되어있다면 setter의
//	접근 권한은 메소드이므로 public을 사용하고 public 권한은 자식 클래스에서 실행할 수 있으므로
//	부모 클래스에서 상속받은 private 권한의 필드를 setter를 이용해서 초기화시킬 수 있다.
//	public Child(String name, boolean gender, int age, String nickname) {
//		setName(name);
//		setGender(gender);
//		this.age = age;
...
//		부모 클래스에서 상속받은 private 권한을 가지는 필드는 자식에서 접근할 수 없으므로
...
//		있으므로 getter를 사용하지 않아도 필드의 내용을 얻어올 수 있다.
