...
public class BoardDao {
	...
	public BoardDto getContent(int idx) {
		...
		try {
			...
			if(rs.next()) {
				...
				board = new BoardDto(idx, writer, pwd, subject, content, writedate, readnum, filename, filesize, homepage, email, refer, depth, step);
			}
	...
	//게시글 상세 (답글 쓰기) 
	public int reWriteOk(BoardDto boardata) {
